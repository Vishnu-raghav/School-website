...
function GalleryImageManager() {
  const [selectedSection, setSelectedSection] = useState("Campus");
  ...
  const sectionOptions = ["Campus", "Event", "Sport", "Lab", "Classroom"];
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white p-8 rounded-xl shadow-lg border border-gray-200 mt-12">
      <h2 className="text-3xl font-bold mb-8 text-blue-900 flex items-center gap-3">
        <span>📷</span> Gallery Image Manager
      </h2>

      <form onSubmit={handleUpload} className="flex flex-col md:flex-row md:items-end gap-6 mb-12">
        <div className="flex-1">
          <label className="block text-sm font-semibold mb-2 text-gray-700">Select Section</label>
          <select
            value={selectedSection}
            onChange={(e) => setSelectedSection(e.target.value)}
            disabled={isUploading}
            className="w-full border px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          >
        ...
        <div className="flex-1">
          <label className="block text-sm font-semibold mb-2 text-gray-700">Choose Image</label>
          <input
            type="file"
            accept="image/*"
            onChange={handleImageChange}
            disabled={isUploading}
            className="w-full border px-4 py-2 rounded-lg disabled:cursor-not-allowed disabled:opacity-50"
          />
        </div>
      ...
      <h3 className="text-3xl font-semibold mb-8 text-blue-900 flex items-center gap-3">
        <span>🖼️</span> Uploaded Images in {selectedSection}
      </h3>

      {galleryImages.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">
          No gallery images uploaded yet in this section.
        </p>
      ) : (
